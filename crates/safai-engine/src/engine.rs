...
//!
//! Callers: Tauri commands, background scheduler, and `safai-cli`. All share
//! the same guardrails and deletion path. Progress is delivered through a
//! `&(dyn Fn(Event) + Send + Sync)` sink so callers can plug in either a
//! Tauri Channel, a CLI no-op, or an in-process aggregator.
...
use safai_rules::{run_scan, CleanupItem, SafetyTier, ScanConfig, ScanEvent, ScanReport};

use crate::delete::{DeleteEvent, DeletePlan, DeletePlanItem, DeleteReport};
...
/// Dry-run: resolve ids against the last-scan map, re-check the allow-list,
/// and report what would happen without touching the disk.
pub fn preview_delete(
    ids: &[String],
    items: &HashMap<String, CleanupItem>,
    roots: &[PathBuf],
) -> DeletePlan {
    let mut plan_items: Vec<DeletePlanItem> = Vec::new();
    ...
    for id in ids {
        match items.get(id) {
            Some(item) => {
                ...
                plan_items.push(DeletePlanItem {
                    id: item.id.clone(),
